<template>
  <div>
    <a-form-model ref="form" :model="model" :rules="validatorRules" class="password-retrieval-form">
      <a-form-model-item label="账号名" v-bind="layout" :labelCol="{span: 6}" :wrapperCol="{span: 18}">
        <a-input type="text" :value="accountName" disabled/>
      </a-form-model-item>

      <a-form-model-item prop="password" label="新密码" v-bind="layout" class="stepFormText" :labelCol="{span: 6}" :wrapperCol="{span: 18}">
        <a-input v-model="model.password" type="password" autocomplete="false"/>
      </a-form-model-item>

      <a-form-model-item prop="confirmPassword" label="确认密码" v-bind="layout" class="stepFormText" :labelCol="{span: 6}" :wrapperCol="{span: 18}">
      ...
      <a-form-model-item :wrapperCol="{span: 19, offset: 5}">
        ...
        <a-button style="margin-left: 20px" @click="skipStep" v-if="skip">跳过</a-button>
...
//    },
    ...
    methods: {
      skipStep() {
        ...
        this.$emit('nextStep', this.userList)
      },
      nextStep () {
        let that = this
        that.loading = true
